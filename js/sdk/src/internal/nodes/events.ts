...
export function nodesEvents(
    ...
    events.registerHandler(async (event) => {
        if (event.type === 'node_created' || event.type === 'node_updated' || event.type === 'node_updated_metadata') {
            await Promise.all(listeners.map(async ({ condition, callback }) => {
                if (condition(event)) {
    ...
    function subscribeToSharedNodesByMe(callback: NodeEventCallback) {
        listeners.push({ condition: ({ isShared }) => isShared || false, callback });
    }

    function subscribeToTrashedNodes(callback: NodeEventCallback) {
        listeners.push({ condition: ({ isTrashed }) => isTrashed || false, callback });
    }

    // TODO: subscription to children needs info about parent - if parent is matching, it will fetch and decrypt
    function subscribeToChildren(parentNodeUid: string, callback: NodeEventCallback) {
        listeners.push({ condition: ({ parentNodeUid: parent }) => parent === parentNodeUid, callback });
    }

    return {
        subscribeToSharedNodesByMe,
        subscribeToTrashedNodes,
        subscribeToChildren,
    }
}
